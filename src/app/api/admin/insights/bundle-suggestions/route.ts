...
import { getBundleSuggestions, initializeAnalyticsTables } from '@/lib/analytics-db';
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get bundle suggestions from analytics database
    const suggestions = await getBundleSuggestions(range, limit);

    return NextResponse.json({
      success: true,
      suggestions,
      range,
      algorithm: 'Co-viewing pattern analysis',
